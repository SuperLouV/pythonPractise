...
from douban_spider_keshe.aiqing_Movie.aiqing_url import get_aiqing_url_list

for final_url in get_aiqing_url_list():
    try:
        # print(final_url)
        ...
        # aiqing_info = html.xpath('/html//div[@class="info"]/span/a')
        aiqing_director = html.xpath('//span[@class="attrs"]/a[1]/text()')
        try:
            aiqing_director_director = aiqing_director[0]
            aiqing_director_screenwriter = aiqing_director[1]
            aiqing_type = html.xpath('//span[@property="v:genre"]/text()')
            aiqing_runtime_list = html.xpath('//span[@property="v:runtime"]/text()')
            aiqing_runtime = aiqing_runtime_list[0]
            aiqing_year = html.xpath('//span[@class="year"]/text()')
            year = aiqing_year[0].strip('(').strip(')')
            aiqing_name_list = html.xpath('//span[@property="v:itemreviewed"]/text()')
            aiqing_name = aiqing_name_list[0]
            aiqing_info_list = html.xpath('//div[@id="info"]//text()')  # 倒数第20位是地区
            score = html.xpath('//div[@class="rating_self clearfix"]/strong/text()')  # 分数
            aiqing_score = score[0]
            # aiqing_area=aiqing_area_list[-20]
            area_num = aiqing_info_list.index('制片国家/地区:')
            actor_num = aiqing_info_list.index('主演')
            language_num = aiqing_info_list.index('语言:')
            aiqing_actor1 = aiqing_info_list[actor_num + 2]  # 主演1位置
            aiqing_actor2 = aiqing_info_list[actor_num + 4]  # 主演2位置
            aiqing_actor3 = aiqing_info_list[actor_num + 6]  # 主演3位置
            aiqing_language = aiqing_info_list[language_num + 1]  # 语言位置
            area = area_num + 1
            aiqing_area = aiqing_info_list[area]  # 地区位置
            area = aiqing_area.strip(' ')
            # // span[ @ property = "v:genre"]
            print("-" * 100)
            # print(len(aiqing_info))
            print(aiqing_director)
            # print(aiqing_info_list)
            print("电影名称：", aiqing_name)
            print("导演为:", aiqing_director_director)
            print("编剧为：", aiqing_director_screenwriter)
            print("主演为：", aiqing_actor1, aiqing_actor2, aiqing_actor3)
            print("电影类型：", aiqing_type)
            print("片长：", aiqing_runtime)
            print("年份:", year)
            print("国家和地区：", area)
            print("语言：", aiqing_language)
            print("分数:", aiqing_score)
            aiqinglist = [aiqing_name, '', aiqing_director_director, '', aiqing_director_screenwriter, '',
                          aiqing_actor1,
                          '', aiqing_actor2, '', aiqing_actor3, aiqing_type, aiqing_runtime, year, area,
                          aiqing_language,
                          aiqing_score]

            # ..........................................
            # 开始存储CSV
            with open('doubanaiqing1000.csv', 'a', encoding='utf-8', newline='') as csvfile:
                writer = csv.writer(csvfile)
                # writer.writerow(['电影名称', '导演', '编剧','主演1','主演2','主演3','电影类型','片长','年份','国家和地区','语言'])
                writer.writerow(aiqinglist)
        except:
