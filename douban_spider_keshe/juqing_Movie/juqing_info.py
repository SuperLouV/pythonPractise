...
from douban_spider_keshe.juqing_Movie.juqing_url import get_juqing_url_list
...
#///////////////////////////////////////////////////////////////////////////////////////
    ...
    try:
        ...
        juqing_type = html.xpath('//span[@property="v:genre"]/text()')
        juqing_runtime_list = html.xpath('//span[@property="v:runtime"]/text()')
        juqing_runtime = juqing_runtime_list[0]
        juqing_year = html.xpath('//span[@class="year"]/text()')
        year = juqing_year[0].strip('(').strip(')')
        juqing_name_list = html.xpath('//span[@property="v:itemreviewed"]/text()')
        juqing_name = juqing_name_list[0]
        juqing_info_list = html.xpath('//div[@id="info"]//text()')  # 倒数第20位是地区
        score = html.xpath('//div[@class="rating_self clearfix"]/strong/text()')  # 分数
        juqing_score = score[0]
        # juqing_area=juqing_area_list[-20]
        area_num = juqing_info_list.index('制片国家/地区:')
        actor_num = juqing_info_list.index('主演')
        language_num = juqing_info_list.index('语言:')
        juqing_actor1 = juqing_info_list[actor_num + 2]  # 主演1位置
        juqing_actor2 = juqing_info_list[actor_num + 4]  # 主演2位置
        juqing_actor3 = juqing_info_list[actor_num + 6]  # 主演3位置
        juqing_language = juqing_info_list[language_num + 1]  # 语言位置
        area = area_num + 1
        juqing_area = juqing_info_list[area]  # 地区位置
        area = juqing_area.strip(' ')
        # // span[ @ property = "v:genre"]
        print("-" * 100)
        # print(len(juqing_info))
        print(juqing_director)
        # print(juqing_info_list)
        print("电影名称：", juqing_name)
        print("导演为:", juqing_director_director)
        print("编剧为：", juqing_director_screenwriter)
        print("主演为：", juqing_actor1, juqing_actor2, juqing_actor3)
        print("电影类型：", juqing_type)
        print("片长：", juqing_runtime)
        print("年份:", year)
        print("国家和地区：", area)
        print("语言：", juqing_language)
        print("分数:", juqing_score)
        juqinglist = [juqing_name, '', juqing_director_director, '', juqing_director_screenwriter, '', juqing_actor1,
                      '', juqing_actor2, '', juqing_actor3, juqing_type, juqing_runtime, year, area, juqing_language,
                      juqing_score]

        # ..........................................
        # 开始存储CSV
        with open('test01.csv', 'a', encoding='utf-8') as csvfile:
