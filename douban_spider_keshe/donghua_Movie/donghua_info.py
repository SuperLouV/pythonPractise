...
from douban_spider_keshe.donghua_Movie.donghua_url import get_donghua_url_list

for final_url in get_donghua_url_list():
    try:
        # print(final_url)
        ...
        # donghua_info = html.xpath('/html//div[@class="info"]/span/a')
        donghua_director = html.xpath('//span[@class="attrs"]/a[1]/text()')
        try:
            donghua_director_director = donghua_director[0]
            donghua_director_screenwriter = donghua_director[1]
            donghua_type = html.xpath('//span[@property="v:genre"]/text()')
            donghua_runtime_list = html.xpath('//span[@property="v:runtime"]/text()')
            donghua_runtime = donghua_runtime_list[0]
            donghua_year = html.xpath('//span[@class="year"]/text()')
            year = donghua_year[0].strip('(').strip(')')
            donghua_name_list = html.xpath('//span[@property="v:itemreviewed"]/text()')
            donghua_name = donghua_name_list[0]
            donghua_info_list = html.xpath('//div[@id="info"]//text()')  # 倒数第20位是地区
            score = html.xpath('//div[@class="rating_self clearfix"]/strong/text()')  # 分数
            donghua_score = score[0]
            # donghua_area=donghua_area_list[-20]
            area_num = donghua_info_list.index('制片国家/地区:')
            actor_num = donghua_info_list.index('主演')
            language_num = donghua_info_list.index('语言:')
            donghua_actor1 = donghua_info_list[actor_num + 2]  # 主演1位置
            donghua_actor2 = donghua_info_list[actor_num + 4]  # 主演2位置
            donghua_actor3 = donghua_info_list[actor_num + 6]  # 主演3位置
            donghua_language = donghua_info_list[language_num + 1]  # 语言位置
            area = area_num + 1
            donghua_area = donghua_info_list[area]  # 地区位置
            area = donghua_area.strip(' ')
            # // span[ @ property = "v:genre"]
            print("-" * 100)
            # print(len(donghua_info))
            print(donghua_director)
            # print(donghua_info_list)
            print("电影名称：", donghua_name)
            print("导演为:", donghua_director_director)
            print("编剧为：", donghua_director_screenwriter)
            print("主演为：", donghua_actor1, donghua_actor2, donghua_actor3)
            print("电影类型：", donghua_type)
            print("片长：", donghua_runtime)
            print("年份:", year)
            print("国家和地区：", area)
            print("语言：", donghua_language)
            print("分数:", donghua_score)
            donghualist = [donghua_name, '', donghua_director_director, '', donghua_director_screenwriter, '',
                          donghua_actor1,
                          '', donghua_actor2, '', donghua_actor3, donghua_type, donghua_runtime, year, area,
                          donghua_language,
                          donghua_score]

            # ..........................................
            # 开始存储CSV
            with open('doubandonghua1000.csv', 'a', encoding='utf-8', newline='') as csvfile:
                writer = csv.writer(csvfile)
                # writer.writerow(['电影名称', '导演', '编剧','主演1','主演2','主演3','电影类型','片长','年份','国家和地区','语言'])
                writer.writerow(donghualist)
        except:
