...
def get_kehuan_url_list():
       ...
       base_url1 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=100%3A90&action=&start={}&limit=20"  # 100-90的影片   完成
       base_url2 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=90%3A80&action=&start={}&limit=20"  # 完成
       base_url3 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=80%3A70&action=&start={}&limit=20"  # 80-70的影片
       base_url4 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=70%3A60&action=&start={}&limit=20"
       base_url5 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=60%3A50&action=&start={}&limit=20"
       base_url6 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=50%3A40&action=&start={}&limit=20"
       base_url7 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=40%3A30&action=&start={}&limit=20"
       base_url8 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=30%3A20&action=&start={}&limit=20"
       base_url9= "https://movie.douban.com/j/chart/top_list?type=17&interval_id=20%3A10&action=&start={}&limit=20"
       base_url10 = "https://movie.douban.com/j/chart/top_list?type=17&interval_id=10%3A0&action=&start={}&limit=20"
       base_url = [base_url1, base_url2, base_url3, base_url4, base_url5, base_url6, base_url7, base_url8, base_url9,
                   base_url10, ]
       # print(base_url)
       num = 0
       kehuan_Movie = []
       while num <= 100:  # num从0 意味着一共20部电影,一般都是每个段位120多电影
           try:
               i = 0
               while i <= 9:  # 第2次设置为1  从80-0开始第二次 4.11  //4，17设置为9全部
                   try:
                       ...
                       print(html)
                       dict_kehuan = json.loads(html_str)  # 先把对象转换为json才可以操作
                       print(type(dict_kehuan))  # 判断类型  list类型。每个list[]里面都是一个字典
                       kehuan_Movie = kehuan_Movie + dict_kehuan
                       print(len(kehuan_Movie))

                       for kehuan in dict_kehuan:
                           print(kehuan, "\n")
                       # 输出电影信息
                       print("第", i, "轮爬虫")
                   except:
       ...
       url_dict_kehuan = []
       title_dict_kehuan = []
       # print(type(kehuan_Movie[2]))
       while j_num < len(kehuan_Movie):
           dict = kehuan_Movie[j_num]
           # url_dict_kehuan[j_num]=dict['url']
           j_num += 1
           url_dict_kehuan.append(dict['url'])
           title_dict_kehuan.append(dict['title'])
           # print(dict['url'])
           print(url_dict_kehuan)
           # 打印url地址
           # print(title_dict_kehuan)
       return url_dict_kehuan
