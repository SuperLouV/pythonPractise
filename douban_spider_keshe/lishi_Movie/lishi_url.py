...
def get_lishi_url_list():
       ...
       num = 0
       lishi_Movie = []
       while num <= 60:  # num从0 意味着一共20部电影,一般都是每个段位80多电影
           try:
               i = 0
               while i <= 9:  # 第2次设置为1  从80-0开始第二次 4.11  //4，17设置为9全部
                   try:
                       ...
                       print(html)
                       dict_lishi = json.loads(html_str)  # 先把对象转换为json才可以操作
                       print(type(dict_lishi))  # 判断类型  list类型。每个list[]里面都是一个字典
                       lishi_Movie = lishi_Movie + dict_lishi
                       print(len(lishi_Movie))

                       for lishi in dict_lishi:
                           print(lishi, "\n")
                       # 输出电影信息
                       print("第", i, "轮爬虫")
                   except:
       ...
       url_dict_lishi = []
       title_dict_lishi = []
       # print(type(lishi_Movie[2]))
       while j_num < len(lishi_Movie):
           dict = lishi_Movie[j_num]
           # url_dict_lishi[j_num]=dict['url']
           j_num += 1
           url_dict_lishi.append(dict['url'])
           title_dict_lishi.append(dict['title'])
           # print(dict['url'])
           print(url_dict_lishi)
           # 打印url地址
           # print(title_dict_lishi)
       return url_dict_lishi
