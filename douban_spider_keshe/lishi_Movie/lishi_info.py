...
from douban_spider_keshe.lishi_Movie.lishi_url import get_lishi_url_list

for final_url in get_lishi_url_list():
    try:
        # print(final_url)
        ...
        # lishi_info = html.xpath('/html//div[@class="info"]/span/a')
        lishi_director = html.xpath('//span[@class="attrs"]/a[1]/text()')
        try:
            lishi_director_director = lishi_director[0]
            lishi_director_screenwriter = lishi_director[1]
            lishi_type = html.xpath('//span[@property="v:genre"]/text()')
            lishi_runtime_list = html.xpath('//span[@property="v:runtime"]/text()')
            lishi_runtime = lishi_runtime_list[0]
            lishi_year = html.xpath('//span[@class="year"]/text()')
            year = lishi_year[0].strip('(').strip(')')
            lishi_name_list = html.xpath('//span[@property="v:itemreviewed"]/text()')
            lishi_name = lishi_name_list[0]
            lishi_info_list = html.xpath('//div[@id="info"]//text()')  # 倒数第20位是地区
            score = html.xpath('//div[@class="rating_self clearfix"]/strong/text()')  # 分数
            lishi_score = score[0]
            # lishi_area=lishi_area_list[-20]
            area_num = lishi_info_list.index('制片国家/地区:')
            actor_num = lishi_info_list.index('主演')
            language_num = lishi_info_list.index('语言:')
            lishi_actor1 = lishi_info_list[actor_num + 2]  # 主演1位置
            lishi_actor2 = lishi_info_list[actor_num + 4]  # 主演2位置
            lishi_actor3 = lishi_info_list[actor_num + 6]  # 主演3位置
            lishi_language = lishi_info_list[language_num + 1]  # 语言位置
            area = area_num + 1
            lishi_area = lishi_info_list[area]  # 地区位置
            area = lishi_area.strip(' ')
            # // span[ @ property = "v:genre"]
            print("-" * 100)
            # print(len(lishi_info))
            print(lishi_director)
            # print(lishi_info_list)
            print("电影名称：", lishi_name)
            print("导演为:", lishi_director_director)
            print("编剧为：", lishi_director_screenwriter)
            print("主演为：", lishi_actor1, lishi_actor2, lishi_actor3)
            print("电影类型：", lishi_type)
            print("片长：", lishi_runtime)
            print("年份:", year)
            print("国家和地区：", area)
            print("语言：", lishi_language)
            print("分数:", lishi_score)
            lishilist = [lishi_name, '', lishi_director_director, '', lishi_director_screenwriter, '',
                          lishi_actor1,
                          '', lishi_actor2, '', lishi_actor3, lishi_type, lishi_runtime, year, area,
                          lishi_language,
                          lishi_score]

            # ..........................................
            # 开始存储CSV
            with open('doubanlishi1000.csv', 'a', encoding='utf-8', newline='') as csvfile:
                writer = csv.writer(csvfile)
                # writer.writerow(['电影名称', '导演', '编剧','主演1','主演2','主演3','电影类型','片长','年份','国家和地区','语言'])
                writer.writerow(lishilist)
        except:
